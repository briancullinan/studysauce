...
<button class="field-add-more-submit ajax-processed" name="field_reminders_add_more" value="Add new"
        onclick="jQuery('#dates .row').first().addClass('edit'); jQuery(this).hide(); jQuery('#dates').addClass('edit-date-only').scrollintoview(); ">
    Add <span>&nbsp;</span> new
</button>
...
<div class="row invalid" id="new-dates-row">
    ...
    <p style="margin-bottom:0;" class="highlighted-link">
        <a href="#save-dates" class="more">Save</a>
    </p>
    <a href="#" onclick="jQuery('#dates').removeClass('edit-date-only').scrollintoview(); jQuery('#dates .field-add-more-submit').show(); jQuery(this).parents('.row').removeClass('edit'); return false;" class="fancy-close">&nbsp;</a>
</div>
<?php
global $user;
...
if (!empty($entities['node'])) {
    ...
    if (isset($node->field_reminders[LANGUAGE_NONE][0]['value']))
    {
        ...
        foreach ($node->field_reminders[LANGUAGE_NONE] as $i => $reminder)
        {
            ...
            if (!empty($entity))
            {
                $time = strtotime($entity[$eid]->field_due_date['und'][0]['value']);
                if($headStr != date('j F', $time))
                {
                    $headStr = date('j F', $time);
                    ?><div class="head"><?php print $headStr; ?></div><?
                }
                $reminders = array_map(function ($x) { return $x['value']; }, $entity[$eid]->field_reminder['und']);
                $classI = array_search($entity[$eid]->field_class_name['und'][0]['value'], array_values($classes));
                ?>
                <div class="row <?php print $first && !($first = false) ? 'first' : ''; ?>" id="eid-<?php print $eid; ?>">
                    <div class="field-type-text field-name-field-class-name field-widget-text-textfield form-wrapper">
                        <div class="read-only"><span class="class<?php print $classI; ?>">&nbsp;</span><?php print htmlspecialchars($entity[$eid]->field_class_name['und'][0]['value'], ENT_QUOTES); ?></div>
                        <div class="form-item form-type-select">
                            <label>&nbsp;</label>
                            <select class="text-full form-select" name="dates-class-name" size="1">
                                ...
                                foreach ($classes as $j => $c) {
                                    ?>
                                    <option value="<?php print htmlspecialchars($c, ENT_QUOTES); ?>" <?php print $entity[$eid]->field_class_name['und'][0]['value'] == $c ? 'selected' : ''; ?>><?php print htmlspecialchars($c, ENT_QUOTES); ?></option><?php
                                }
                                ?>
                                <option value="Nonacademic">Nonacademic</option>
                            </select>
                    ...
                    <div class="field-type-text field-name-field-assignment field-widget-text-textfield form-wrapper">
                        <div class="read-only"><label>Assignment</label><?php print htmlspecialchars($entity[$eid]->field_assignment['und'][0]['value'], ENT_QUOTES); ?></div>
                        <div class="form-item form-type-textfield">
                            <label for="edit-field-reminders-und-1-field-assignment-und-0-value">Assignment </label>
                            <input class="text-full form-text jquery_placeholder-processed" placeholder="Paper, exam, project, etc."
                                   type="text" name="dates-assignment" value="<?php print htmlspecialchars($entity[$eid]->field_assignment['und'][0]['value'], ENT_QUOTES); ?>" size="60" maxlength="255">
                        </div>
                    </div>
                    <div class="field-type-list-integer field-name-field-reminder field-widget-options-buttons form-wrapper">
                    ...
                    <div class="field-type-number-integer field-name-field-percent field-widget-number form-wrapper">
                        <div class="read-only"><label>% of grade</label><?php print $entity[$eid]->field_percent['und'][0]['value']; ?>%</div>
                        <div class="form-item form-type-textfield">
                            <label for="dates-percent">% of grade </label>
                            <input type="text" name="dates-percent" value="<?php print $entity[$eid]->field_percent['und'][0]['value']; ?>" size="12" maxlength="10" class="form-text">
                        </div>
                    </div>
                    <div class="field-type-list-boolean field-name-field-completed field-widget-options-onoff form-wrapper">
